...
class Adam(Optimizer):
    ...
    def step(self):
        ...
        for param, grad, m, v in zip(parameters, gradients, self.state['m'], self.state['v']):
            ...
            np.add(self.beta1 * m,  gr, out = m) # Здесь может быть проблема как в прошлый раз
            
            np.add(self.beta2 * v,  (gr ** 2), out = v)
            
            m_hat = m  / (1 - self.beta1 ** t) # тут может быть проблема что ха бета1 с крышкой??
            
            v_hat = v / (1 - self.beta2 ** t)
            
            np.add(param, self.lr * m_hat / (np.sqrt(v_hat) + self.eps))
