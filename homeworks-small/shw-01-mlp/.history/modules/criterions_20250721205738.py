...
class CrossEntropyLoss(Criterion):
    ...
    def compute_output(self, input: np.ndarray, target: np.ndarray) -> float:
        ...
        n1 = (probs - np.log(sum_probs))
        
        mask = target
        
        if target.ndim == 1:                      # ещё не маска
            C = input.shape[1]                    # число классов
            mask = np.eye(C, dtype=input.dtype)[target]    # (B, C) one‑hot
        
        n2 = n1 * mask
        
        n3 = np.sum(n2)
        
        loss = - 1 / (input.shape[0]) * n3
        
        target = np.longdouble(target)
        
        return np.longdouble(loss)
