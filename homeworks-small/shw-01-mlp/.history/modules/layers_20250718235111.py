...
class BatchNormalization(Module):
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        if self.training:
            ...
            output = gamma / (input.shape[0] * np.sqrt(self.running_var + self.eps)) * (input.shape[0] * grad_output - np.sum(grad_output, axis=0) - self.norm_input * np.sum(grad_output*self.norm_input, axis=0)) 
            return output
